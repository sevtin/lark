...
func (r *RedisSingle) HSetNXEx(key, field string, value interface{}, ex time.Duration) (err error) {
	key = RealKey(key)
	var (
		pipe = r.client.Pipeline()
...
func (r *RedisSingle) CHMSet(key string, values map[string]string, expire time.Duration) (err error) {
	var (
		pipe = r.client.Pipeline()
	)
	key = RealKey(key)
...
func (r *RedisSingle) CHDel(maps map[string][]string) (err error) {
	if len(maps) == 0 {
		return
	}
	var (
