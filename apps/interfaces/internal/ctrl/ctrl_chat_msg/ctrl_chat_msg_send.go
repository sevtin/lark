...
func (ctrl *ChatMessageCtrl) SendChatMessage(ctx *gin.Context) {
	var (
		params   = new(dto_chat_msg.SendChatMessageReq)
		uid      int64
		platform int32
		resp     *xhttp.Resp
		err      error
	...
	resp = ctrl.chatMessageService.SendChatMessage(params, uid, platform)
	if resp.Code > 0 {
